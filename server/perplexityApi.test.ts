...
describe("Perplexity API Service", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Reset env
    delete process.env.APEX_ENGINE_API_KEY;
  });

  describe("isApexEngineConfigured", () => {
    it("should return false when API key is missing", () => {
      delete process.env.APEX_ENGINE_API_KEY;
      expect(isApexEngineConfigured()).toBe(false);
    });

    it("should return true when API key is set", () => {
      process.env.APEX_ENGINE_API_KEY = "test-api-key";
      expect(isApexEngineConfigured()).toBe(true);
    });
  });
  ...
  describe("executeApexAnalysis", () => {
    it("should throw error when API key is not configured", async () => {
      ...
      await expect(executeApexAnalysis("Test problem")).rejects.toThrow(
        "APEX_ENGINE_API_KEY is not configured"
      );
    });
